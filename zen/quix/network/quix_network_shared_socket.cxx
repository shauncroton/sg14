...
{ static
  void
  fd_listener_fn( const std::string& fd_listener_arg,
      const std::string& fd_receiver_arg,
    const std::string& fd_sender_arg)
    { zen::quix_network_shared_listener fd_listener( fd_listener_arg,
      fd_receiver_arg,
      fd_sender_arg);
    fd_listener();}
  ...
  static
  void
  fd_sender_fn( const std::string& fd_sender_arg,
 ...
 fd_sender();
  ...
  { address addr_fd_listener;
    ...
    std::thread fd_listener_thread( plumbing::fd_listener_fn,
    ...
    std::thread fd_sender_thread( plumbing::fd_sender_fn,
      addr_fd_sender.quix_messaging_unix(),
      sender_buff);
 ...
 fd_receiver();
    ...
    auto client_recvd =
 client.recv( client_recv_buff, 1024 );

        std::string client_send_buff( "by the sea shore" );
    auto client_sent =
 client.send( client_send_buff.data(), client_send_buff.size() );

    char receiver_recv_buff[1024];
    memset( receiver_recv_buff, 0, 1024 );
    auto receiver_recvd =
 receiver->recv( receiver_recv_buff, 1024 );

    fd_listener_thread.join();
        fd_sender_thread.join();

    CHECK( sender_buff.size() ==
 client_recvd );
    CHECK( sender_buff ==
 std::string( client_recv_buff, client_recvd ) );

    CHECK( client_send_buff.size() ==
 client_sent );
    CHECK( client_send_buff.size() ==
 receiver_recvd );
    CHECK( client_send_buff ==
 std::string( receiver_recv_buff, receiver_recvd ) );}
