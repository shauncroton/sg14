...
{ static
  void
  listener( std::promise< void > &vprom,
...
template< typename A >
  ...
  { zen::quix_network_socket_listening< A > listening( address_arg );
    ...
    auto recvd  =
 connected.recv( buff, 1024 );
    auto sent =
 connected.send( buff, recvd );
    if( recvd !=
 sent ) throw std::logic_error( "received size did not match sent size" );}

  static
  void
  echo_fd( std::promise< void > &vprom,
    const std::string address_arg)
  { zen::quix_network_socket_listening< zen::quix_network_address_unix > listening( address_arg );
    ...
    char buff[1024]; memset( buff, 0, 1024 );
    auto recvd =
 connected.recv_fd( buff, 1024, fd );
    auto sent =
 connected.send_fd( buff, recvd, fd );
    if( recvd !=
 sent ) throw std::logic_error( "received size did not match sent size" );}

  static
  void
  connector( std::future< void > &vfut,
    const std::string address_arg)
  { vfut.get();
      std::this_thread::yield();
 ...
 vprom.get_future();
    ...
    auto send_size =
 connected.send( send_buff.c_str(), send_buff.size() );

    char recv_buff[1024]; memset( recv_buff, 0, 1024 );
    auto recv_size =
 connected.recv( recv_buff, 1024 );
 ...
 recv_size );
    CHECK( send_buff ==
 std::string( recv_buff, recv_size ) );}
 ...
 vprom.get_future();
    ...
    auto send_size =
 connected.send_fd( send_buff.c_str(), send_buff.size(), send_fd );

    char recv_buff[1024]; memset( recv_buff, 0, 1024 );
    int recvd_fd;
    auto recv_size =
 connected.recv_fd( recv_buff, 1024, recvd_fd );
    ...
    auto send_size_inet =
 connected_inet.send( send_buff_inet.c_str(), send_buff_inet.size() );

    char recv_buff_inet[1024]; memset( recv_buff_inet, 0, 1024 );
    auto recv_size_inet =
 connected_inet.recv( recv_buff_inet, 1024 );
 ...
 std::string( recv_buff, recv_size ) );
    ...
    CHECK( send_size_inet ==
 recv_size_inet );
    CHECK( send_buff_inet ==
 std::string( recv_buff_inet, recv_size_inet ) );}}
api;
///
///////////////////////////////////////////////////////////////////////////////////////////////////
///
