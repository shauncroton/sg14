...
{ plumbing()
  ...
  { std::string address_arg( "quix_memory_localhost:12345" );
        auto address =
 std::make_quix_memory_shared< zen::quix_network_address_inet >( address_arg );}}
plumbing;
///
///////////////////////////////////////////////////////////////////////////////////////////////////
///
...
{ api()
 ...
 AF_INET;
    sa.sin_addr.s_addr =
 inet_addr( "quix_memory_localhost" );
    sa.sin_port =
 htons( 12345 );

    CHECK( address.addr()->sa_family ==
 reinterpret_cast< struct sockaddr& >( sa ).sa_family );
    const struct sockaddr_in *address_ptr =
 reinterpret_cast< const struct sockaddr_in* >( address.addr() ) ;
    CHECK( address_ptr->sin_family ==
 sa.sin_family );
    CHECK( address_ptr->sin_addr.s_addr ==
 sa.sin_addr.s_addr );
    CHECK( address_ptr->sin_port ==
 sa.sin_port );}
